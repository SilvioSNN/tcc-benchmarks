...
#pragma acc parallel loop copyin(A[0:MAT_LEN][0:MAT_LEN], B[0:MAT_LEN][0:MAT_LEN]) \
                          copy(C[0:MAT_LEN][0:MAT_LEN]) //collapse(2)
    for (i = 0; i < MAT_LEN; ++i)
        for (j = 0; j < MAT_LEN; ++j)
            for (k = 0; k < MAT_LEN; ++k)
                C[i][j] += A[i][k] * B[k][j];
...
#define RESULT_FILE_NAME "matmul_acc.rslt"
#endif
#ifndef PRECISION
#define PRECISION 6
#endif
